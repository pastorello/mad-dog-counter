...
- **Vettoriali di brand**: la tipografia non li fornirà, quindi restano da ridisegnare a mano a partire dai raster in `design/raw/`. **Eccezione**: il logo House of Mad Dogs (bicchiere + wordmark + tagline) ha ora un vettoriale vero, consegnato dal committente il 2026-08-22 (`design/raw/sottobicchiere_homd_93x93.pdf`) — `design/processed/homd_shot_glass.svg` e `ShotGlassPainter` in `lib/ui/widgets/homd_mark.dart` sono trascritti path per path da lì, non più ricalcati a occhio. Il font del wordmark ("HOUSE OF MAD DOGS", tagline) è confermato **Oswald Bold** (`kBrandFont`), preso dal font incorporato nello stesso PDF. Gli altri asset (Ciommo, bandiera Dutch) restano da ridisegnare dai raster; gli SVG già ricavati stanno in `design/processed/`.
